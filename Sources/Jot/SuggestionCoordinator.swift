...
final class SuggestionCoordinator {
    ...
    func request() {
        ...
        card.show(text: "Using recent Jot context…", loading: true, at: frame)
        ...
        requestTask = Task { [weak self] in
            ...
            do {
                ...
                let scenario = context.input(target: target, association: .explicitRecentRequest)
                ...
                case .output(let raw):
                    ...
                    case .suggestion(let text):
                        guard !SuggestionOutput.isFieldEcho(text, draft: field.draft, placeholder: field.placeholder) else {
                            self.showNotice("No suggestion: the result repeated the field or its placeholder."); return
                        }
