...
enum AppleFMGeneration {
    /// Keep equal to the Package.swift pin; a test checks it.
    static let revision = "737fac9e7147403f2777e0901f02452e8fc25ae7"
    static let sampling = "greedy"

    static var availability: String { AppleFMClient().modelAvailability.rawValue }

    /// Each call is a fresh AppleFM session with no retained conversation state.
    static func generate(_ request: ModelRequest) async throws -> String {
        #if canImport(FoundationModels)
