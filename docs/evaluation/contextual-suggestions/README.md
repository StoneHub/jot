...
## Evaluation harness (source prepared, not yet compiled or run)

`jot-suggestion-eval` ([`Sources/JotSuggestionEvaluation`](../../../Sources/JotSuggestionEvaluation), [#86](https://github.com/StoneHub/jot/issues/86)) runs this corpus against the on-device model. It was written on Linux without a Swift compiler. Until the Mac gate below passes, it is unverified source, and no results exist. It reads only the corpus you name and writes only to a new output directory. It never connects to Jot, reads transcripts, inserts text or runs commands.
