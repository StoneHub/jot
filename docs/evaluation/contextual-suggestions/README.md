...
4. Store one record per scenario and run: `scenarioId`, `run` (`cold` or `warm`), `selectedSources`, `outcome` (`suggest`, `abstain`, `invalidate`, `error` or `timeout`), verbatim `outputText`, `timeToPreviewMs`, `scores` per applicable criterion (`pass`, `fail` or `not-applicable`), `scorer` and `notes`.
