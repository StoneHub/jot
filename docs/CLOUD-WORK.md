...
A PASS report is validation evidence, not merge authorization. The script verifies that the PR is open and still points to the checked commit before posting; it also rejects a checkout changed during checks. The local integrator owns review, merge, canonical checkout updates and authorized installation under `AGENTS.md`. Cloud workers stop after the scoped handoff; do not start polling, recurring checks or auto-merge loops.

Copy-ready prompt for a local agent:

> Check Jot PR #N. From the Jot checkout, run `python3 scripts/local-pr-check.py N --dry-run`, then `python3 scripts/local-pr-check.py N --post --note "<what you reviewed>"`. Before posting, do the manual review items in the PR description and summarize them in the note. If a gate fails, investigate in `work/pr-N`. For a small fix within the PR's scope, commit there, push with the command the script prints, and run the check again. Otherwise post the failing evidence and the fix you propose. Complete any merge and installation already authorized by the active task, following `AGENTS.md` and the capture-safe installer. Otherwise return the validation report as the handoff.
