...
Per-job records measure continuous recognition: audio duration, queue wait, inference time when available, and outcome. Held dictation now selects that same saved timeline; it does not submit a separate long recognition job on release. The dictation-started/released lifecycle markers remain, but recognition job latency is not release-to-field latency. Failed and cancelled jobs remain visible and are excluded from latency summaries. Percentile values from a few jobs are not stable benchmarks.
