...
Cloud lane: source preparation with portable checks. The current Swift package requires Apple frameworks, so Linux cannot compile the package or run the model. Do not install Swift to discover this again. Mac validation and actual evaluation belong to the local integrator.
